/****************************************************************************
** Meta object code from reading C++ file 'AI.h'
**
** Created: Thu Apr 12 17:44:14 2012
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../AI.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'AI.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_AI[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
       4,    3,    3,    3, 0x0a,

 // methods: signature, parameters, type, tag, flags
      23,   14,    3,    3, 0x02,
      57,   52,   48,    3, 0x02,
      69,    3,   48,    3, 0x02,
      81,    3,    3,    3, 0x02,
     100,   89,   48,    3, 0x02,

       0        // eod
};

static const char qt_meta_stringdata_AI[] = {
    "AI\0\0quitapp()\0i,j,mark\0setPosition(int,int,int)\0"
    "int\0mark\0winner(int)\0checkfull()\0"
    "print()\0randommode\0play(int)\0"
};

void AI::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        AI *_t = static_cast<AI *>(_o);
        switch (_id) {
        case 0: _t->quitapp(); break;
        case 1: _t->setPosition((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2])),(*reinterpret_cast< int(*)>(_a[3]))); break;
        case 2: { int _r = _t->winner((*reinterpret_cast< int(*)>(_a[1])));
            if (_a[0]) *reinterpret_cast< int*>(_a[0]) = _r; }  break;
        case 3: { int _r = _t->checkfull();
            if (_a[0]) *reinterpret_cast< int*>(_a[0]) = _r; }  break;
        case 4: _t->print(); break;
        case 5: { int _r = _t->play((*reinterpret_cast< int(*)>(_a[1])));
            if (_a[0]) *reinterpret_cast< int*>(_a[0]) = _r; }  break;
        default: ;
        }
    }
}

const QMetaObjectExtraData AI::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject AI::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_AI,
      qt_meta_data_AI, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &AI::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *AI::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *AI::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_AI))
        return static_cast<void*>(const_cast< AI*>(this));
    return QObject::qt_metacast(_clname);
}

int AI::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
